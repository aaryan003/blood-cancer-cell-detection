...
export function ModelMetrics() {
  const [metrics] = useState<PerformanceMetric[]>([
    { title: "Accuracy", value: "--", description: "Overall model accuracy", icon: Target, color: "blue", trend: "--" },
    { title: "Precision", value: "--", description: "Positive prediction accuracy", icon: Zap, color: "purple", trend: "--" },
    { title: "Recall", value: "--", description: "True positive detection rate", icon: Activity, color: "green", trend: "--" },
    { title: "F1-Score", value: "--", description: "Harmonic mean of precision and recall", icon: TrendingUp, color: "orange", trend: "--" },
  ]);
    ...
    <div className="max-w-6xl space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {metrics.length === 0 ? (
          <div className="col-span-4">
            <p className="text-gray-400 text-sm py-8 text-center">No metrics data available</p>
          </div>
        ) : (
          metrics.map((metric) => {
      ...
      <Card className="p-6">
        ...
        {confusionMatrix === null ? (
          <p className="text-gray-400 text-sm py-8 text-center">No confusion matrix data available</p>
        ) : (
